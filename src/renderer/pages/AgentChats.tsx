  ...
  return (
    <div className="flex h-full -m-6 overflow-hidden">
      ...
      <aside className="flex w-64 flex-shrink-0 flex-col border-r border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex flex-shrink-0 items-center border-b border-slate-200 px-3 py-3 dark:border-zinc-800">
        ...
        <div className="flex-shrink-0 border-b border-slate-100 px-3 py-2 dark:border-slate-700/50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-200 bg-slate-50 py-1.5 pl-7 pr-2 text-xs text-slate-900 outline-none focus:border-slate-400 dark:border-zinc-700 dark:bg-zinc-800/50 dark:text-slate-100"
            />
          </div>
        </div>

        {(agentsError || chatroomsError) && (
          <div className="flex items-start gap-1.5 border-b border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700 dark:border-zinc-700 dark:bg-zinc-800/50 dark:text-slate-300">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          {loadingAgents && agents.length === 0 ? (
            <div className="space-y-2 px-3 py-3">
              {[0, 1, 2].map((i) => (
                <div key={i} className="h-12 rounded bg-slate-100 dark:bg-zinc-700/40 shimmer" />
              ))}
            </div>
          ) : agents.length === 0 ? (
              ...
              {filtered.map((agent) => {
                ...
                return (
                  <li key={agent.id}>
                    ...
                    >
                      <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-100 text-xs font-bold text-slate-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-slate-300">
                      ...
                      <div className="min-w-0 flex-1">
                        <p
                          className={cn(
                            ...
                            isActive ? 'text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-200',
                          )}
                        >
                          {agent.name}
                        </p>
        ...
        {!agentId ? (
          ...
          <div className="flex flex-1 items-center justify-center">
            <div className="flex flex-col items-center gap-3 text-slate-500">
              <span className="inline-block h-6 w-6 animate-spin rounded-full border-2 border-slate-200 border-t-slate-700" />
          ...
          <div className="flex flex-1 items-center justify-center p-8">
            <div className="card max-w-md p-8 text-center">
              <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 text-slate-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-slate-400">
          ...
          <>
            <header className="flex flex-shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-5 py-3 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100 text-sm font-bold text-slate-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-slate-300">
                {showAvatar ? (
                  <img
            ...
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto bg-slate-50 px-4 py-6 scrollbar-thin dark:bg-zinc-950"
            >
                ...
                {activeMessages
                  ...
                  .map((m) => (
                    <MessageBubble
                      ...
                      onRegenerate={() => chatRoomId && removeMessage(chatRoomId, m.id)}
                      onDelete={() => chatRoomId && removeMessage(chatRoomId, m.id)}
                    />
                  ))}
...
const EmptyAgentChat: React.FC<{ onCreate: () => void }> = ({ onCreate }) => (
  <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
    ...
    <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Select an agent</h2>
    <p className="mt-2 max-w-sm text-sm text-slate-500">
      Pick an agent from the left to start a private 1:1 conversation. Memory and history are kept per agent.
    </p>
    <div className="mt-8 grid max-w-sm grid-cols-3 gap-2 text-center">
      {[
        { icon: <Bot size={16} />, label: 'Private 1:1' },
        { icon: <Brain size={16} />, label: 'Long memory' },
        { icon: <Sparkles size={16} />, label: 'Custom LLM' },
      ].map((item) => (
        <div key={item.label} className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-zinc-700 dark:bg-zinc-800">
          <div className="flex justify-center text-slate-400 dark:text-slate-500">{item.icon}</div>
